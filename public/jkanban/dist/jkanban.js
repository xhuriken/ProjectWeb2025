            ...
            this.jKanban = function () {
                ...
                this.addBoards = function (boards, isInit) {
                    ...
                    for (var boardkey in boards) {
                        ...
                        // if window.userRole == 'admin';, add delete btn

                        if (window.userRole === 'admin') {
                            headerBoard.innerHTML = `
                              <div class="kanban-title-board">${board.title}</div>
                              <button class="ml-auto group" type="button">
                                <i class="ki-filled ki-trash group-hover:text-red-500 duration-300"></i>
                              </button>
                            `;
                        } else {
                            // user n’est pas admin => pas de poubelle
                            headerBoard.innerHTML = `
                              <div class="kanban-title-board">${board.title}</div>
                            `;
                        }
